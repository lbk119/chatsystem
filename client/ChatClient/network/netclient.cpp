...
void NetClient::initWebsocket()
{
    ...
    connect(&websocketClient, &QWebSocket::binaryMessageReceived, this, [=](const QByteArray& byteArray) {
        ...
        lbk::NotifyMessage notifyMessage;
...
void NetClient::handleWsResponse(const lbk::NotifyMessage &notifyMessage)
{
    if (notifyMessage.notifyType() == lbk::NotifyTypeGadget::NotifyType::CHAT_MESSAGE_NOTIFY) {
        // 收到消息
    ...
    } else if (notifyMessage.notifyType() == lbk::NotifyTypeGadget::NotifyType::CHAT_SESSION_CREATE_NOTIFY) {
        // 创建新的会话通知
        ChatSessionInfo chatSessionInfo;
        chatSessionInfo.load(notifyMessage.newChatSessionInfo().chatSessionInfo());
        handleWsSessionCreate(chatSessionInfo);
    } else if (notifyMessage.notifyType() == lbk::NotifyTypeGadget::NotifyType::FRIEND_ADD_APPLY_NOTIFY) {
        // 添加好友申请通知
        UserInfo userInfo;
        userInfo.load(notifyMessage.friendAddApply().userInfo());
        handleWsAddFriendApply(userInfo);
    } else if (notifyMessage.notifyType() == lbk::NotifyTypeGadget::NotifyType::FRIEND_ADD_PROCESS_NOTIFY) {
        // 添加好友申请的处理结果通知
        UserInfo userInfo;
    ...
    } else if (notifyMessage.notifyType() == lbk::NotifyTypeGadget::NotifyType::FRIEND_REMOVE_NOTIFY) {
        // 删除好友通知
...
{
    lbk::ClientAuthenticationReq req;
...
void NetClient::getMyself(const QString &loginSessionId)
{
    ...
    connect(httpResp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto resp = handleHttpResponse<lbk::GetUserInfoRsp>(httpResp, &ok, &reason);
...
void NetClient::getFriendList(const QString& loginSessionId)
{
    ...
    connect(httpResp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto friendListResp = this->handleHttpResponse<lbk::GetFriendListRsp>(httpResp, &ok, &reason);
...
void NetClient::getChatSessionList(const QString &loginSessionId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::GetChatSessionListRsp>(resp, &ok, &reason);
...
void NetClient::getApplyList(const QString &loginSessionId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::GetPendingFriendEventListRsp>(resp, &ok, &reason);
...
void NetClient::getRecentMessageList(const QString &loginSessionId, const QString &chatSessionId, bool updateUI)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::GetRecentMsgRsp>(resp, &ok, &reason);
...
{
    // 1. 通过 protobuf 构造 body
    lbk::NewMessageReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setChatSessionId(chatSessionId);

    // 构造 MessageContent
    lbk::MessageContent messageContent;
    if (messageType == MessageType::TEXT_TYPE) {
        messageContent.setMessageType(lbk::MessageTypeGadget::MessageType::STRING);

        lbk::StringMessageInfo stringMessageInfo;
        ...
        messageContent.setMessageType(lbk::MessageTypeGadget::MessageType::IMAGE);

        lbk::ImageMessageInfo imageMessageInfo;
        ...
        messageContent.setMessageType(lbk::MessageTypeGadget::MessageType::FILE);

        lbk::FileMessageInfo fileMessageInfo;
        ...
        messageContent.setMessageType(lbk::MessageTypeGadget::MessageType::SPEECH);

        lbk::SpeechMessageInfo speechMessageInfo;
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::NewMessageRsp>(resp, &ok, &reason);
...
void NetClient::changeNickname(const QString &loginSessionId, const QString &nickname)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::SetUserNicknameRsp>(resp, &ok, &reason);
...
{
    // 1. 通过 protobuf 构造请求 body
    lbk::SetUserDescriptionReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::SetUserDescriptionRsp>(resp, &ok, &reason);
...
void NetClient::getVerifyCode(const QString &phone)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::PhoneVerifyCodeRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    lbk::SetUserPhoneNumberReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setPhoneNumber(phone);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::SetUserPhoneNumberRsp>(resp, &ok, &reason);
...
void NetClient::changeAvatar(const QString &loginSessionId, const QByteArray &avatar)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::SetUserAvatarRsp>(resp, &ok, &reason);
...
void NetClient::deleteFriend(const QString &loginSessionId, const QString &userId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::FriendRemoveRsp>(resp, &ok, &reason);
...
void NetClient::addFriendApply(const QString &loginSessionId, const QString &userId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::FriendAddRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    lbk::ChatSessionCreateReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::ChatSessionCreateRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    lbk::GetChatSessionMemberReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    pbReq.setChatSessionId(chatSessionId);
    QByteArray body = pbReq.serialize(&serializer);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::GetChatSessionMemberRsp>(resp, &ok, &reason);
...
void NetClient::searchUser(const QString &loginSessionId, const QString &searchKey)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::FriendSearchRsp>(resp, &ok, &reason);
...
void NetClient::searchMessage(const QString &loginSessionId, const QString &chatSessionId, const QString &searchKey)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::MsgSearchRsp>(resp, &ok, &reason);
...
void NetClient::searchMessageByTime(const QString &loginSessionId, const QString &chatSessionId, const QDateTime &begTime, const QDateTime &endTime)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::GetHistoryMsgRsp>(resp, &ok, &reason);
...
void NetClient::userLogin(const QString &username, const QString &password)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::UserLoginRsp>(resp, &ok, &reason);
...
void NetClient::userRegister(const QString &username, const QString &password)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::UserRegisterRsp>(resp, &ok, &reason);
...
void NetClient::phoneLogin(const QString &phone, const QString &verifyCodeId, const QString &verifyCode)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::PhoneLoginRsp>(resp, &ok, &reason);
...
void NetClient::phoneRegister(const QString& phone, const QString& verifyCodeId, const QString& verifyCode) {
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::PhoneRegisterRsp>(resp, &ok, &reason);
...
void NetClient::getSingleFile(const QString &loginSessionId, const QString &fileId)
{
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::GetSingleFileRsp>(resp, &ok, &reason);
...
{
    // 1. 构造请求 body
    lbk::SpeechRecognitionReq pbReq;
    pbReq.setRequestId(makeRequestId());
    pbReq.setSessionId(loginSessionId);
    ...
    connect(resp, &QNetworkReply::finished, this, [=]() {
        ...
        bool ok = false;
        QString reason;
        auto pbResp = this->handleHttpResponse<lbk::SpeechRecognitionRsp>(resp, &ok, &reason);
