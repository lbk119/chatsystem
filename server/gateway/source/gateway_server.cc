...
DEFINE_int32(http_listren_port, 9000, "HTTP服务器监听端口");
DEFINE_int32(websocket_listren_port, 9001, "Websocket服务器监听端口");
...
int main(int argc, char *argv[])
{
    ...
    lbk::GatewayServerBuilder gsb;
    gsb.make_redis_object(FLAGS_redis_host, FLAGS_redis_port, FLAGS_redis_db, FLAGS_redis_keep_alive);
    gsb.make_discovery_object(FLAGS_registry_host, FLAGS_base_service, FLAGS_file_service, FLAGS_friend_service,
                              FLAGS_message_service, FLAGS_transmite_service, FLAGS_user_service, FLAGS_speech_service);
    gsb.make_server_object(FLAGS_websocket_listren_port, FLAGS_http_listren_port);
